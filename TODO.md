...
## 当前状态
**当前阶段**: Phase 4 - 物品和商店系统 ✅ 已完成
**下一步**: Phase 5 存档系统 或 4.2 背包系统
...
- [x] 4.1 物品系统 (`systems/items.py`)
  - [x] 物品基类 (ItemData, ItemEffect)
  - [x] 物品类型枚举 (ItemType: KEY, POTION, WEAPON, ARMOR, SPECIAL)
  - [x] 物品效果系统 (HP/攻击/防御/金币/钥匙等)
  - [x] 物品管理器 (ItemManager, 从JSON加载)
  - [x] 物品数据文件 (`data/entities/items.json`)
- [x] 4.1a 地图集成
  - [x] FloorManager 添加物品加载逻辑
  - [x] 添加物品渲染方法
  - [x] Game 类添加物品拾取逻辑
- [x] 4.1b 地图设计
  - [x] 第1层：入门层（简单怪物 + 基础物品）
...
- [x] 4.1c 物品资源配置
  ...
  - [x] 物品清单:
    - 钥匙: yellow_key, blue_key, red_key, green_key, master_key (5种)
...
- [x] 4.1d 宝石系统
  ...
  - [x] 在前三层地图添加宝石
  - [x] 拾取消息显示属性增加
- [x] 4.3 商店系统 (`systems/shop.py`)
  - [x] 商店数据管理 (ShopData, ShopItem, ShopUpgrade)
  - [x] 商店管理器 (ShopManager, 从JSON加载)
  - [x] 交易逻辑 (购买物品、属性升级)
  - [x] 商店数据文件 (`data/entities/shops.json`)
- [x] 4.4 商店界面 (`ui/shop_ui.py`)
  - [x] ShopUI 组件
  - [x] 物品列表显示
  - [x] 属性升级显示
  - [x] 金币显示和价格颜色
  - [x] 购买确认和消息提示
  - [x] 快捷键操作 (↑↓选择, Enter购买, ESC关闭)
- [x] 4.5 商店集成
  - [x] 地图商店实体支持 (FloorManager._shops)
  - [x] 玩家移动检测商店
  - [x] 游戏状态切换 (PLAYING ↔ SHOP)
  - [x] 购买回调处理
  - [x] 修复商店显示位置问题
  - [x] 修复ESC键无限递归问题
- [ ] 4.6 商店优化 (待讨论)
  - [ ] 购买时不能重复购买（武器/防具是装备，不是消耗品）
  - [ ] 商店界面美化
- [ ] 4.2 背包系统
  - [ ] 背包管理
  - [ ] 物品使用
...
### 商店系统问题 (2026-03-12)
- **商店显示被缩放**: 修复 ShopUI 使用渲染表面尺寸而不是窗口尺寸
- **商店不自动打开**: 将 `_check_shop()` 调用移到 `move_player()` 中
- **ESC键导致无限递归**: 修复 `_close_shop()` 避免调用 `on_close` 回调导致的递归
